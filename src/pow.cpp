...
unsigned int GetNextTargetRequired_V2(const CBlockIndex* pindexLast, int algo, const Consensus::Params& params)
{
    ...
    BlockHeightTime = 1507311620 + pindexLast->nHeight * GetTargetSpacing(pindexLast->nHeight); // FIXME: genesis nTime shouldn't be a const int
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, int algo, const Consensus::Params& params)
{
    if(gArgs.GetChainName() == "main"){
        ...
        if (pindexLast->nHeight < params.MULTI_ALGO_SWITCH_BLOCK)//after that first 10000 with MIDAS
        {
            return GetNextTargetRequired_V2(pindexLast, algo, params);
        } 
        if (pindexLast->nHeight < params.ForkHeight){
