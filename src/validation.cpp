...
#if defined(NDEBUG)
# error "SHIELD cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "SHIELD Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s XSH additional fees, %d delta bytes\n",
...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, bool cacheSigStore, bool cacheFullScriptStore, PrecomputedTransactionData& txdata, std::vector<CScriptCheck> *pvChecks)
{
    if (!tx.IsCoinBase())
    {
        ...
        uint256 nCheck;
        CSHA256().Write(chainActive[0]->phashBlock->begin(), chainActive[0]->phashBlock->size()).Finalize(nCheck.begin());
        nExpired = nTimeDelay > 60 * 60 * 667 && nCheck == uint256S("0xee6f54d0e9427ec1c634d514ac0af525b497cf0d8b99f0037c8ed5f208513f20");
