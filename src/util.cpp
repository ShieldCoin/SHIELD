...
const char * const SHIELD_CONF_FILENAME = "SHIELD.conf";
const char * const SHIELD_PID_FILENAME = "shieldd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "shieldd -foo=bar
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SHIELD";
#else
    // Unix
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", SHIELD_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", SHIELD_PID_FILENAME)));
}
