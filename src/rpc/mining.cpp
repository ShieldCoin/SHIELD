...
static UniValue GetNetworkHashPS(int lookup, int height, int algo = ALGO) {
    const CBlockIndex *pb = GetLastBlockIndex4Algo(chainActive.Tip(), algo);
...
static UniValue getallnetworkhashps(const JSONRPCRequest& request)
{
    ...
    obj.pushKV("x16s", GetNetworkHashPS(blocks, height, ALGO_X16S));
    return obj;
}
...
static UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. address      (string, required) The address to send the newly generated SHIELD to.\n"
...
// NOTE: Unlike wallet RPC (which use XSH values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
