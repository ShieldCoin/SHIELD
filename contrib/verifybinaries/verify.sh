...
###   shieldcore.org and shield.org and compares them.
...
WORKINGDIR="/tmp/shield_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://shieldcore.org"
HOST2="https://shield.org"
BASEDIR="/bin/"
VERSIONPREFIX="shield-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'shield-',
...
if [ $? -ne 0 ]; then
   echo "shield.org failed to provide signature file, but shieldcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "shield.org and shieldcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the SHIELD Core binary release signing key installed?"
   fi
