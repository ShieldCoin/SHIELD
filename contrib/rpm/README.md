...
The RPM spec file provided here is for shield-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://shield.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[SHIELD git repository](https://github.com/shield/shield) but are not part of
...
Currently only `Source10` is used, for the example `shield.conf` file.
...
in the shield git repository. At present only an SVG version of the SHIELD
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the shield
...
needed when 0.12.1 is released, a proper fix is already in the SHIELD git
master branch.
...
idea to build SHIELD Core against a static version of that library compiled
...
The `shield.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `shield-server` package. At this point, most RPM based
...
The `shield-server` package should have SELinux support. How to properly do
...
shield-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/shield/). He has been
packaging SHIELD for Fedora at least since 2012.
...
4. I renamed the `shield` package that contains the Qt GUI to `shield-core` as
that appears to be how the general population refers to it, in contrast to
`shield-xt` or `shield-classic`. I wanted to make sure the general population
